...
int yuv_to_h264_nv12(uint8_t *y_ptr, uint8_t *uv_ptr, uint8_t **h264_data, 
    ...
    if (g_mmz_cnt > 0 && g_mmz_vaddr[idx] != NULL && g_mmz_paddr[idx] != 0) {
        ...
        stFrame.stVFrame.phy_ptr[2] = 0;
        stFrame.stVFrame.size = y_size + uv_size;
        stFrame.stVFrame.width = width;
        stFrame.stVFrame.height = height;
        stFrame.stVFrame.pix_format = HB_PIXEL_FORMAT_NV12; 
...
void vps_small_release(hb_vio_buffer_t* chn_3_out_buf) {
    ...
    HB_VPS_StopGrp(0);
    HB_VPS_DestroyGrp(0);
    fprintf(stderr, "[vps_small_release] VPS released\n");
}
...
int send_session_info_to_receiver(char *receiver) {
    ...
    if (g_start_vts > 0 && strlen(g_peer_name) && strlen(g_remote_server_name) && 
        g_uidn && g_ssrc && g_download_port > 0) {
        
        if (!strcmp(receiver, WEB_AGENT)) {
            strcpy(recver, SRS_AGENT);
            sprintf(callback_msg, "type=6;control_type=7;uidn=%u;ssrc=%u;sid=%s;web_agent=%s;stream_name=%s;download_port=%d", 
                    g_uidn, g_ssrc, g_sid, g_peer_name, g_stream_name, g_download_port);
        } else if (strlen(receiver)) {
            strcpy(recver, receiver);
            sprintf(callback_msg, "type=6;control_type=2;uidn=%u;ssrc=%u;server_name=%s;download_port=%d", 
                    g_uidn, g_ssrc, g_remote_server_name, g_download_port);
        } else {
...
// ============================ Live SDK回调 ============================
// live SDK推流初始化成功回调
void xftpDidStart(long uidn, long ssrc, const char *remoteFilePath, const char *remoteServerName, 
                  int remoteServerPort, int downloadPort) {
    int rt = 0;
...
// live SDK推流结束回调
void xftpTransferSuccess(long uidn, long ssrc, const char *remoteFilePath, const char *remoteServerName, 
                         int remoteServerPort, int downloadPort) {
...
int update_channel_online(int is_online) {
    ...
    snprintf(update_sql_channel, sizeof(update_sql_channel) - 1, 
             "update m_channel set is_normal = %d, is_online = %d where channel_no = '%s'", 
             is_local_normal, is_online, g_channel_no);
    ...
    snprintf(url, sizeof(url) - 1, 
             "http://%s:%d/live/channel/modChannelXtOnline?device_no=%s&is_online_xt=%d&is_normal=%d", 
             g_web_server, g_web_port, g_xftp_user, is_online, is_local_normal);
            ...
            if (item != NULL && item->length) {
                ...
                if (strstr(item->str, "=")) {
                    ...
                    if (item_pairs->items != NULL) {
                        ...
                        if (!key->length || !value->length) break;
...
int read_config_xtvf(const char *channel_no) {
    ...
    // 获取通道号
    snprintf(select_sql_channel, sizeof(select_sql_channel) - 1, 
             "select id,channel_no,channel_pwd,channel_sip,protocol,stream_url from m_channel where channel_no = \"%s\"", 
             channel_no);
