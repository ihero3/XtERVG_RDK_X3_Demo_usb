...
// UVC 拉流：直接 H264 数据传入现有回调，保持后续逻辑一致
void *uvc_h264_thread_func(void *arg)
{
	int ret;
	int uvc_fd = -1;
	...
	if (uvc_fd < 0) {
		fprintf(stderr, "[uvc_h264_thread_func] Failed to open %s: %s\n", g_v_channel, strerror(errno));
		goto exit;
	}
	...
	if (ioctl(uvc_fd, VIDIOC_S_FMT, &fmt) < 0) {
		fprintf(stderr, "[uvc_h264_thread_func] VIDIOC_S_FMT H264 failed: %s\n", strerror(errno));
		goto exit;
	}
	...
	if (ioctl(uvc_fd, VIDIOC_REQBUFS, &req) < 0) {
		fprintf(stderr, "[uvc_h264_thread_func] VIDIOC_REQBUFS failed: %s\n", strerror(errno));
		goto exit;
	}

	buffers = (struct buffer *)calloc(req.count, sizeof(struct buffer));
	if (!buffers) {
		fprintf(stderr, "[uvc_h264_thread_func] Out of memory\n");
		goto exit;
	}
	...
	for (n_buffers = 0; n_buffers < req.count; ++n_buffers) {
		...
		if (ioctl(uvc_fd, VIDIOC_QUERYBUF, &buf) < 0) {
			fprintf(stderr, "[uvc_h264_thread_func] VIDIOC_QUERYBUF failed: %s\n", strerror(errno));
			goto exit;
		}
		...
		if (buffers[n_buffers].start == MAP_FAILED) {
			fprintf(stderr, "[uvc_h264_thread_func] mmap failed: %s\n", strerror(errno));
			goto exit;
		}
	}
	...
	for (unsigned int i = 0; i < n_buffers; ++i) {
		...
		if (ioctl(uvc_fd, VIDIOC_QBUF, &buf) < 0) {
			fprintf(stderr, "[uvc_h264_thread_func] VIDIOC_QBUF failed: %s\n", strerror(errno));
			goto exit;
		}
	}

	if (ioctl(uvc_fd, VIDIOC_STREAMON, &v4l2_type) < 0) {
		fprintf(stderr, "[uvc_h264_thread_func] VIDIOC_STREAMON failed: %s\n", strerror(errno));
		goto exit;
	}

	while (g_is_running && !g_should_exit_main && g_is_uvc_running) {
		fd_set fds;
		struct timeval tv;
		int r;
		...
		if (r < 0) {
			...
			fprintf(stderr, "[uvc_h264_thread_func] select error: %s\n", strerror(errno));
			break;
		} else if (r == 0) {
			continue;
		}
		...
		if (ioctl(uvc_fd, VIDIOC_DQBUF, &buf) < 0) {
			...
			fprintf(stderr, "[uvc_h264_thread_func] VIDIOC_DQBUF failed: %s\n", strerror(errno));
			break;
		}
...
exit:
	...
	fprintf(stderr, "[uvc_h264_thread_func] exit\n");
	return NULL;
}
...
int start_uvc_stream(void)
{
	int ret = 0;
	g_is_uvc_running = 1;
	ret = pthread_create(&g_uvc_thread, NULL, uvc_h264_thread_func, NULL);
	if (ret != 0) {
...
int start_pull_video(void)
{
	...
	if (!strcmp(g_stream_protocol, "rtsp")) {
		...
		if (strlen(g_v_channel) == 0) {
			fprintf(stderr, "[start_pull_video] g_v_channel empty for uvc\n");
			return -4;
		}
		rt = start_uvc_stream();
		if (rt) {
